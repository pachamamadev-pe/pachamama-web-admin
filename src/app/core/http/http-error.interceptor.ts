 ...
 * Muestra notificaciones automáticamente y formatea mensajes de error
 * Los componentes pueden manejar lógica adicional (UI updates) pero no necesitan mostrar notificaciones
 */
    ...
    catchError((error: HttpErrorResponse) => {
      ...
      if (error.error instanceof ErrorEvent) {
        ...
        switch (error.status) {
          ...
          case 401:
            errorMessage =
              error.error?.message || 'Sesión expirada. Por favor, inicia sesión nuevamente.';
          ...
          case 500:
            errorMessage =
              error.error?.message || 'Error interno del servidor. Intenta nuevamente más tarde.';
            break;
          case 503:
            errorMessage =
              error.error?.message ||
      ...
      // Los componentes pueden manejar lógica adicional pero no necesitan mostrar notificaciones
      //if (error.status !== 401) {
      // 401 se maneja en auth.interceptor
