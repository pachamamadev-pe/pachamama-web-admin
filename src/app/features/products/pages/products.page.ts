...
export class ProductsPage implements OnInit {
  ...
  openEditDialog(product: Product): void {
    console.log(
      ...oo_oo(`1095412821_127_4_127_62_4`, 'openEditDialog called for product:', product),
    );
    if (product.icon) {
      const cache = this.imageUrlCache();
      console.log(...oo_oo(`1095412821_130_6_130_42_4`, 'Current cache:', cache));
      // Caso 1: Ya está en caché → abrir dialog inmediatamente
      if (cache.has(product.icon)) {
        console.log(
          ...oo_oo(
            `1095412821_133_8_133_73_4`,
            'Image URL found in cache:',
            cache.get(product.icon),
          ),
        );
  ...
  private openEditDialogWithImage(product: Product, currentImageUrl: string | null): void {
    console.log(
      ...oo_oo(`1095412821_165_4_165_71_4`, 'Opening edit dialog with image URL:', currentImageUrl),
    );
    const dialogRef = this.dialog.open<
      ProductFormDialogComponent,
      ProductFormDialogData,
      ...
      if (result?.dto) {
        ...
        if (result.imagesToDelete && result.imagesToDelete.length > 0) {
          this.azureStorage.deleteFiles(result.imagesToDelete).subscribe({
            ...
            error: (error) => {
              console.error(
                ...oo_tx(`1095412821_191_14_191_64_11`, 'Error deleting old images:', error),
              );
  ...
  getProductImageUrl(iconPath: string | null): string | null {
    ...
    this.azureStorage.getFileUrl(iconPath, 5).subscribe({
      ...
      error: (error) => {
        console.error(
          ...oo_tx(`1095412821_331_8_331_67_11`, 'Error al obtener URL con SAS token:', error),
        );
      },
    });
